...
class AffiliationForm(FlaskForm):
    ...
    Fields:
        organization (StringField): An optional field to input the name of the organization.
    """
...
class AuthorForm(FlaskForm):
    ...
    Fields:
        author_name (StringField): The name of the author. This field is optional.
        affiliations (FieldList): A list of affiliations for the author. This field contains a minimum of 1 and a maximum of 3 AffiliationForm entries.
        email (StringField): The email address of the author. This field is optional.
        orcid (StringField): The ORCID identifier of the author. This field is optional.
    """
...
class FilterSelectionForm(FlaskForm):
    ...
    Fields:
        selected (BooleanField): A boolean field that indicates whether any filters are selected. Defaults to "No filters found!".
        file_name (HiddenField): A hidden field used to store the file name and pass this value to the MMM script. This is a workaround since passing the value via BooleanField was not possible due to HTML limitations with FieldList.
...
class JournalDataForm(FlaskForm):
    ...
    Fields:
        publisher_id (StringField): The ID of the publisher. Required.
        eissn (StringField): The electronic ISSN of the journal. Required.
        title (StringField): The title of the journal. Required.
        publisher_name (StringField): The name of the publisher. Required.
        publisher_loc (StringField): The location of the publisher. Required.
        journal_link (StringField): The URL link to the journal. Required and must be a valid URL.
        license_text (TextAreaField): The text of the license. Required.
        license_type (StringField): The type of the license. Required.
        license_link (StringField): The URL link to the license. Required and must be a valid URL.
        copyright_holder (StringField): The holder of the copyright. Required.
        submit (SubmitField): The submit button to update the form.
    """
    # Journal section
...
class UploadForm(FlaskForm):
    ...
    Fields:
        markdown (FileField): Required field for uploading a Markdown file. Only accepts files with extensions 'md' or 'markdown'.
        metadata (FileField): Optional field for uploading a metadata file. Only accepts files with extensions 'yaml' or 'yml'.
        bibtex (FileField): Optional field for uploading a BibTeX file. Only accepts files with extension 'bib'.
        images (MultipleFileField): Optional field for uploading multiple image files. Only accepts files with extensions 'png', 'jpg', or 'jpeg'.
        filter_selection (FieldList): Dynamic form field for selecting filters. Displays all filters in the filter folder.
        file_name (StringField): Optional field for specifying individual file names.
        pdf (BooleanField): Checkbox to select PDF output.
        html (BooleanField): Checkbox to select HTML output.
        jats (BooleanField): Checkbox to select JATS output.
        latex (BooleanField): Checkbox to select LaTeX output.
        manual_data_input (BooleanField): Checkbox to activate manual data input instead of uploading a metadata file.
        submit (SubmitField): Button to submit the form and create files.

    Optional Metadata Fields (used if no metadata file is uploaded):
        title (StringField): Optional field for the article title.
        subtitle (StringField): Optional field for the article subtitle.
        keywords (FieldList): Optional field for article keywords. Can have between 1 and 8 entries, each being a StringField.
        abstract (TextAreaField): Optional field for the article abstract.
        date (StringField): Optional field for the article year.
        volume (IntegerField): Optional field for the article volume.
        doi (StringField): Optional field for the article DOI.
        author_short (StringField): Optional field for a short author description.
        authors (FieldList): Optional field for author data. Can have between 1 and 20 entries, each being an AuthorForm.
    """
