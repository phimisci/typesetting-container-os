"""
Script to start the application. Called by the Dockerfile via ENTRYPOINT. 
Web interface is started if argument --webinterface is passed. In this case, 
you need to run the Docker container with the -p 5000:5000 option to access the 
web interface. If any other arguments besides --webinterace are passed, the 
script md2files.py is run with these arguments. In this case, you need to mount 
the article folder you are currently in via -v $(pwd):/app/article
"""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Start Typesetting Container: Either Webinterface or " \
    ...
    parser.add_argument(
        ...
        help="The metadata file to use. This file should be in YAML format and " \
             "it should be procuded by XML2YAML based on an OJS article " \
             "metadata file."
        )
    parser.add_argument(
        "--bibtex_file", 
    ...
    parser.add_argument(
        ...
        help="Pandoc Lua-filters to use. These filters must be stored in the " \
             "subfolder 'filter/'. Please provide the filter name WITH the file " \
             "extension."
        )
    parser.add_argument(
        "--widows", 
    ...
    parser.add_argument(
        "--pdf", 
        action="store_true", 
        help="Generate PDF file based on a template " \
        "from the subfolder called 'templates/'.")
    parser.add_argument(
        "--proof", 
        action="store_true", 
        help="Generate proof PDF file based on a template from the subfolder " \
        "called 'templates/'.")
    parser.add_argument(
        "--filename", 
        type=str, 
...
def main(args) -> None:
    ...
    if args.webinterface:
        ...
        if args.compounds:
            command.append("--compounds")
        if args.parentheses:
            command.append("--parentheses")
        if args.html:
            command.append("--html")
        if args.pdf:
            command.append("--pdf")
